...
- 更新检查界面改为先显示“正在检查更新…”，只有收到真实的最新版本结果后才显示“当前已是最新版本”，避免打开时产生默认最新的误解。
- `1.1.3`：重新封装发布手动更新版本；检测到新版本时只提示，点击“下载更新”后才开始下载。

- 更新策略改为手动下载：检测到新版本时仅提示，用户点击“下载更新”后才开始下载，不再自动占用后台网络和磁盘。
...
- 修复牛本体独立缩放后的布局重叠：放大牛时同步预留 stage 高度，避免牛头侵入行情卡片。
- 调整大小精调：窗口与行情/UI 保持 100% 可读，大小滑杆仅缩放牛本体，允许牛独立变小。
- `1.0.12`：检测到更新后自动后台下载，无需关闭应用或再次点击下载；完成后再提示重启安装。
- `1.0.11`：自动更新下载进度文案改为“作者手搓中”，并保留百分比进度。
- `1.0.10`：检查更新确认当前已是最新版本时，按钮显示“已经最新了，别催了”。
